...
class StandardLoader(BaseLoader):
    ...
    def train_test_split(
        ...
        """
        Split interactions into train/test using a strategy.
        """
        if strategy == "random":
            return self._random_split(df, test_size, random_state)

        elif strategy == "user_stratified":
            return self._user_stratified(df, test_size, random_state)

    def _random_split(self, df, test_size, random_state):
        return train_test_split(df, test_size=test_size, random_state=random_state)

    def _user_stratified(self, df, test_size, random_state):
        # ensures each user appears in both splits
